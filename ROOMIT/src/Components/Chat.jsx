...
// const Chat = React.memo(({ userData = [] }) => {
...
//         ) : (
//           messages.map((msg, i) => (
//             <div key={i} className={`chat-message ${msg.senderId === myId ? "right" : "left"}`}>
...
const Chat = ({ userData = [] }) => {
  ...
  const otherUserId = getOtherUserId(roomId);
  ...
  const sendMessage = () => {
    ...
    axios
      .post(`/api/chat/room/${roomId}/message`, newMessage)
      .then((res) => {
